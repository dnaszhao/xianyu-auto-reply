...
class XianyuLive:
    ...
    try:
        _password_login_cooldown = max(10, int(os.getenv("PASSWORD_LOGIN_COOLDOWN_SECONDS", "60")))
    except Exception:
        _password_login_cooldown = 60
    # 在冷却期内仍出现 Session 过期时，允许在达到该阈值后应急重试一次
    try:
        _password_login_emergency_retry_after = max(
            5,
            int(os.getenv("PASSWORD_LOGIN_EMERGENCY_RETRY_SECONDS", "20"))
        )
    except Exception:
        _password_login_emergency_retry_after = 20
    ...
    def __init__(self, cookies_str=None, cookie_id: str = "default", user_id: int = None):
        ...
        self.proactive_token_refresh_min_interval = 3600  # 主动刷新最小间隔：1小时
        self.proactive_token_refresh_max_interval = 7200  # 主动刷新最大间隔：2小时
        self.next_proactive_token_refresh_time = 0
        self._schedule_next_proactive_token_refresh("初始化")
        ...
        self._restart_requested = False  # 标记当前实例是否已触发重启流程
        self._last_password_refresh_action = "none"  # 记录最近一次密码刷新动作
                    ...
                    if isinstance(res_json, dict):
                        ...
                        if '令牌过期' in res_json_str or 'Session过期' in res_json_str:
                            ...
                            if not refresh_success:
                                ...
                                # 如果已触发实例重启，避免当前实例继续递归刷新（会与重启流程竞态）
                                refresh_action = getattr(self, "_last_password_refresh_action", "")
                                if refresh_action == "restart_triggered" or getattr(self, "_restart_requested", False):
                                    logger.info(
                                        f"【{self.cookie_id}】密码登录已触发实例重启，当前实例停止递归刷新，等待新实例接管"
                                    )
                                    return None
                                # 刷新成功后，重新尝试获取token
    ...
    async def _handle_captcha_verification(self, res_json: dict) -> str:
        ...
        try:
            ...
            try:
                ...
                slider_headless_env = os.getenv("SLIDER_HEADLESS")
                if slider_headless_env is None:
                    slider_headless = not bool(os.getenv("DISPLAY"))
                else:
                    slider_headless = slider_headless_env.strip().lower() in ("1", "true", "yes", "on")

                logger.info(
                    f"【{self.cookie_id}】滑块浏览器模式: "
                    f"{'headless' if slider_headless else 'headed'}"
                )
                ...
                slider_stealth = XianyuSliderStealth(
                    ...
                    initial_cookies_str=self.cookies_str
                )
                ...
                if success and cookies:
                    ...
                    # 优先使用滑块验证返回的完整Cookie集合做候选，避免只更新x5导致主流程继续使用旧Cookie
                    updated_cookies = self.cookies.copy()
                    total_new_cookie_count = 0
                    total_updated_cookie_count = 0
                    for cookie_name, cookie_value in cookies.items():
                        if cookie_name in updated_cookies:
                            if updated_cookies[cookie_name] != cookie_value:
                                ...
                                total_updated_cookie_count += 1
                        else:
                            updated_cookies[cookie_name] = cookie_value
                            total_new_cookie_count += 1

                    logger.info(
                        f"【{self.cookie_id}】滑块完整Cookie合并完成: "
                        f"新增{total_new_cookie_count}个, 更新{total_updated_cookie_count}个, 总计{len(updated_cookies)}个"
                    )
                    ...
                    for cookie_name, cookie_value in x5sec_cookies.items():
                        existing_cookie_value = self.cookies.get(cookie_name)
                        if existing_cookie_value is not None:
                            if existing_cookie_value != cookie_value:
                    ...
                    # 将完整候选cookies字典转换为字符串格式
                    cookies_str = "; ".join([f"{k}={v}" for k, v in updated_cookies.items()])

                    logger.info(
                        f"【{self.cookie_id}】x5 Cookie统计完成: 新增{new_cookie_count}个, "
                        f"更新{updated_cookie_count}个, 完整Cookie总计{len(updated_cookies)}个"
                    )

                    x5sec_cookies_str = "; ".join([f"{k}={v}" for k, v in x5sec_cookies.items()]) if x5sec_cookies else "无"

                    # 先校验候选Cookie是否真的可用，避免伪更新
                    validation_result = await self._validate_candidate_cookies_for_persistence(
                        updated_cookies,
                        source="滑块验证"
                    )
                    if not validation_result.get('valid'):
                        logger.warning(f"【{self.cookie_id}】滑块验证后的候选Cookie未通过校验，拒绝落库")
                        log_captcha_event(
                            self.cookie_id,
                            "滑块验证成功但Cookie校验失败，未更新数据库",
                            False,
                            f"原因: {validation_result.get('details', '未知')}, x5 cookies: {x5sec_cookies_str}"
                        )
                        await self.send_token_refresh_notification(
                            f"滑块验证成功，但Cookie校验失败，未覆盖原Cookie\n原因: {validation_result.get('details', '未知')}",
                            "captcha_cookie_validation_failed"
                        )
                        return None
                    ...
                    try:
                        ...
                        # 记录成功更新到日志文件，包含x5相关的cookie信息和校验结果
                        log_captcha_event(self.cookie_id, "滑块验证成功并自动更新数据库", True,
                            f"cookies长度: {len(cookies_str)}, 新增{total_new_cookie_count}个cookie, 更新{total_updated_cookie_count}个cookie, "
                            f"新增{new_cookie_count}个x5, 更新{updated_cookie_count}个x5, 总计{len(updated_cookies)}个cookie项, "
                            f"校验结果: {validation_result.get('details', '通过')}, x5 cookies: {x5sec_cookies_str}")

                        # 发送成功通知
                        await self.send_token_refresh_notification(
                            f"滑块验证成功，且Cookie校验通过，已自动更新到数据库",
    ...
    async def _update_cookies_and_restart(self, new_cookies_str: str):
        ...
        try:
            ...
            try:
                ...
                # 身份字段兜底：不生成伪值，仅在实例已有稳定 user_id 时回填 unb
                merged_unb = str(merged_cookies_dict.get('unb', '') or '').strip()
                if not merged_unb:
                    user_id_fallback = str(getattr(self, 'user_id', '') or '').strip()
                    if user_id_fallback and user_id_fallback.isdigit() and len(user_id_fallback) >= 8:
                        merged_cookies_dict['unb'] = user_id_fallback
                        updated_fields.append('unb(由user_id回填)')
                        logger.warning(
                            f"【{self.cookie_id}】合并Cookie时检测到unb缺失，已使用当前实例user_id安全回填"
                        )
                ...
                # 最终门禁：关键字段不完整时，拒绝覆盖当前Cookie
                missing_critical = self._get_missing_critical_cookie_keys(new_cookies_dict)
                if missing_critical:
                    logger.error(
                        f"【{self.cookie_id}】合并后Cookie仍缺关键字段 {missing_critical}，拒绝更新并保持原Cookie"
                    )
                    return False
    ...
    async def _try_password_login_refresh(self, trigger_reason: str = "令牌/Session过期"):
        ...
        Returns:
            bool: 是否成功刷新Cookie
        """
        self._last_password_refresh_action = "none"
        ...
        if last_password_login > 0 and time_since_last_login < XianyuLive._password_login_cooldown:
            ...
            is_session_expired_trigger = ("Session过期" in trigger_reason) or ("令牌过期" in trigger_reason)
            can_emergency_retry = (
                is_session_expired_trigger and
                time_since_last_login >= XianyuLive._password_login_emergency_retry_after
            )
            if can_emergency_retry:
                logger.warning(
                    f"【{self.cookie_id}】仍在冷却期内（剩余{remaining_time:.1f}秒），"
                    f"但持续触发{trigger_reason}，已达到应急重试阈值"
                    f"({XianyuLive._password_login_emergency_retry_after}秒)，继续执行密码登录刷新"
                )
            else:
        ...
        try:
            ...
            if db_cookie_value and db_cookie_value != self.cookies_str:
                ...
                self._last_password_refresh_action = "db_reload"
                return True
            ...
            display_available = _is_display_available()
            # 仅在显示服务可用时才强制切有头模式，避免 DISPLAY 存在但 Xvfb 不可用导致启动失败。
            if not show_browser and display_available:
                if os.getenv("PASSWORD_LOGIN_HEADLESS", "false").strip().lower() not in ("1", "true", "yes", "on"):
                    show_browser = True
                    logger.info(f"【{self.cookie_id}】检测到可用DISPLAY环境，密码登录切换为有头模式")
            elif show_browser and not display_available:
                logger.warning(f"【{self.cookie_id}】配置为有头模式，但DISPLAY不可用，回退为无头模式")
                show_browser = False
            ...
            # 在独立线程池中运行同步 Playwright，避免复用线程导致 "inside asyncio loop" 错误
            import concurrent.futures
            slider = XianyuSliderStealth(user_id=self.cookie_id, enable_learning=False, headless=not show_browser)
            loop = asyncio.get_running_loop()
            with concurrent.futures.ThreadPoolExecutor(max_workers=1, thread_name_prefix=f"pwd_login_{self.cookie_id}") as executor:
                result = await loop.run_in_executor(
                    executor,
                    lambda: slider.login_with_password_playwright(
                        account=username,
                        password=password,
            ...
            if result:
                ...
                # 强校验：仅当“与当前Cookie合并后”关键字段完整才允许继续更新
                merged_preview = self.cookies.copy()
                merged_preview.update(result)
                missing_after_merge = self._get_missing_critical_cookie_keys(merged_preview)
                if missing_after_merge:
                    logger.error(
                        f"【{self.cookie_id}】密码登录候选Cookie关键字段不完整（合并后仍缺失: {missing_after_merge}），拒绝更新"
                    )
                    await self.send_token_refresh_notification(
                        f"密码登录获取到的Cookie缺少关键字段，已拒绝更新: {', '.join(missing_after_merge)}",
                        "password_login_cookie_incomplete"
                    )
                    return False
                ...
                if update_success:
                    # 仅在Cookie更新并触发重启成功后记录冷却时间，避免伪成功阻塞后续补救
                    XianyuLive._last_password_login_time[self.cookie_id] = time.time()
                    if getattr(self, "_restart_requested", False):
                        self._last_password_refresh_action = "restart_triggered"
                    else:
                        self._last_password_refresh_action = "cookie_updated"
                    logger.warning(
                        f"【{self.cookie_id}】已记录密码登录时间，冷却期 {XianyuLive._password_login_cooldown} 秒，"
                        f"应急重试阈值 {XianyuLive._password_login_emergency_retry_after} 秒"
                    )
                    ...
                    self._last_password_refresh_action = "update_failed"
                    return False
                    
            else:
                logger.warning(f"【{self.cookie_id}】密码登录失败，未获取到Cookie")
                self._last_password_refresh_action = "login_failed"
                return False
            ...
            self._last_password_refresh_action = "exception"
            return False
    ...
    async def _verify_cookie_validity(self) -> dict:
        ...
        strict_image_validation = os.getenv(
            "COOKIE_IMAGE_UPLOAD_STRICT_VALIDATION", "false"
        ...
        try:
            ...
            try:
                ...
                if upload_result:
                    ...
                    # 上传失败不一定代表Cookie失效，可能是上传链路风控/临时限制
                    logger.warning(f"【{self.cookie_id}】⚠️ 图片上传API验证失败: 上传失败（可能是风控或Cookie失效）")
                    result['image_api'] = False
                    if strict_image_validation:
                        result['valid'] = False
                        result['details'].append("图片上传API: 上传失败（严格模式判定为无效）")
                    else:
                        result['details'].append("图片上传API: 上传失败（非严格模式，暂不判定Cookie失效）")
            ...
            if strict_image_validation:
                result['valid'] = False
                result['details'].append(f"图片上传API: 验证异常（严格模式）- {error_str[:50]}")
            else:
                result['details'].append(f"图片上传API: 验证异常（非严格模式）- {error_str[:50]}")
    ...
    async def _validate_candidate_cookies_for_persistence(self, candidate_cookies: dict, source: str = "unknown") -> dict:
        """验证候选Cookie是否适合持久化落库。

        只要关键字段缺失或真实API验证失败，就拒绝覆盖当前Cookie，避免伪更新。
        """
        validation_result = {
            'valid': False,
            'missing_keys': [],
        ...
        missing_keys = self._get_missing_critical_cookie_keys(candidate_cookies)
        validation_result['missing_keys'] = missing_keys
        if missing_keys:
            validation_result['details'] = f"缺少关键字段: {', '.join(missing_keys)}"
            logger.warning(f"【{self.cookie_id}】{source}候选Cookie校验失败，{validation_result['details']}")
            return validation_result

        original_cookies_str = self.cookies_str
        original_cookies = self.cookies.copy()

        try:
            self.cookies = candidate_cookies.copy()
            self.cookies_str = '; '.join([f"{k}={v}" for k, v in self.cookies.items()])

            api_result = await self._verify_cookie_validity()
            validation_result['api_result'] = api_result

            if api_result.get('valid'):
                validation_result['valid'] = True
                validation_result['details'] = api_result.get('details', 'Cookie验证通过')
                logger.info(f"【{self.cookie_id}】{source}候选Cookie通过有效性验证")
            else:
                validation_result['details'] = api_result.get('details', '关键API验证失败')
                logger.warning(f"【{self.cookie_id}】{source}候选Cookie未通过有效性验证: {validation_result['details']}")
        finally:
            self.cookies_str = original_cookies_str
    ...
    def _should_attempt_token_refresh(self, current_time: float):
        """判断当前是否真的需要刷新Token。"""
        if not self.current_token:
            return True, "当前没有可用Token"

        token_age = current_time - self.last_token_refresh_time if self.last_token_refresh_time else float('inf')

        if token_age >= self.token_refresh_interval:
            return True, "Token已达到强制刷新时机"

        if self.next_proactive_token_refresh_time and current_time >= self.next_proactive_token_refresh_time:
            return True, "达到随机主动刷新时间窗口"
        ...
        if not ws_connected:
            return True, "WebSocket未连接，刷新Token用于重连"

        return False, "未到随机主动刷新窗口，且连接状态正常"

    def _schedule_next_proactive_token_refresh(self, reason: str = "unknown"):
        """安排下一次1-2小时随机Token主动刷新。"""
        delay = random.randint(
            self.proactive_token_refresh_min_interval,
            self.proactive_token_refresh_max_interval
        )
        self.next_proactive_token_refresh_time = time.time() + delay
        next_refresh_str = time.strftime(
            "%Y-%m-%d %H:%M:%S",
            time.localtime(self.next_proactive_token_refresh_time)
        )
        logger.info(
            f"【{self.cookie_id}】已安排下一次随机Token主动刷新: {next_refresh_str} "
            f"(约{delay // 60}分钟后，原因: {reason})"
        )

    def _should_attempt_cookie_refresh(self, current_time: float):
        """判断当前是否真的需要刷新Cookie。"""
        missing_keys = self._get_missing_critical_cookie_keys(self.cookies)
        if missing_keys:
            return True, f"Cookie缺少关键字段: {', '.join(missing_keys)}"
        ...
        if not self.current_token and not ws_connected:
            return True, "当前无Token且WebSocket未连接，尝试刷新Cookie恢复会话"

        return False, "未检测到Cookie异常或恢复需求"
        ...
        try:
            while True:
                try:
                    ...
                    current_time = time.time()
                    should_refresh, refresh_reason = self._should_attempt_token_refresh(current_time)
                    if not should_refresh:
                        logger.info(f"【{self.cookie_id}】跳过本轮Token刷新: {refresh_reason}")
                        await self._interruptible_sleep(60)
                        continue

                    logger.info(f"【{self.cookie_id}】满足Token刷新条件，开始刷新: {refresh_reason}")
                    new_token = await self.refresh_token()
                    if new_token:
                        self._schedule_next_proactive_token_refresh("Token刷新成功")
            ...
            while True:
                try:
                    ...
                    if current_time - self.last_cookie_refresh_time >= self.cookie_refresh_interval:
                        should_refresh, refresh_reason = self._should_attempt_cookie_refresh(current_time)
                        if not should_refresh:
                            logger.info(f"【{self.cookie_id}】跳过本轮Cookie刷新: {refresh_reason}")
                        ...
                        # 检查是否已有Cookie刷新任务在执行
                            ...
                            logger.info(f"【{self.cookie_id}】满足Cookie刷新条件，开始执行: {refresh_reason}")
    ...
    def _get_missing_critical_cookie_keys(self, cookies_dict: dict) -> list:
        critical_keys = ['unb', '_m_h5_tk', '_m_h5_tk_enc', 'cookie2', 't', 'sgcookie', 'cna']
        missing = []
        for key in critical_keys:
            value = (cookies_dict or {}).get(key)
            if not value:
                missing.append(key)
        return missing

    async def _collect_full_cookie_snapshot_async(self, context, page, timeout: float = 18.0) -> dict:
        """通过多域访问、共享请求与 storage_state 尽可能抓完整 Cookie。"""
        target_urls = [
        ...
        while time.time() - start_time < timeout:
            for target_url in target_urls:
                if time.time() - start_time >= timeout:
                    break
                try:
                    logger.info(f"【{self.cookie_id}】全量Cookie采集中，访问页面: {target_url}")
                    await page.goto(target_url, wait_until='domcontentloaded', timeout=12000)
                    await asyncio.sleep(1)
                except Exception as visit_error:
                    logger.warning(f"【{self.cookie_id}】访问 {target_url} 失败，继续采集: {self._safe_str(visit_error)}")

                try:
                    response = await context.request.get(target_url, timeout=12000, fail_on_status_code=False)
                    logger.info(f"【{self.cookie_id}】共享Cookie请求完成: {target_url} -> {response.status}")
                except Exception as request_error:
                    logger.debug(f"【{self.cookie_id}】共享Cookie请求失败 {target_url}: {self._safe_str(request_error)}")
                ...
                except Exception as context_cookie_error:
                    logger.debug(f"【{self.cookie_id}】读取 context.cookies() 失败: {self._safe_str(context_cookie_error)}")
                ...
                try:
                    ...
                    logger.debug(f"【{self.cookie_id}】读取 storage_state 失败: {self._safe_str(storage_state_error)}")

                try:
                    document_cookie = await page.evaluate("() => document.cookie") or ""
                    for cookie_pair in document_cookie.split("; "):
                        if "=" in cookie_pair:
                            name, value = cookie_pair.split("=", 1)
                            if name and value:
                                merged_cookies[name] = value
                except Exception as document_cookie_error:
                    logger.debug(f"【{self.cookie_id}】读取 document.cookie 失败: {self._safe_str(document_cookie_error)}")

            missing = self._get_missing_critical_cookie_keys(merged_cookies)
            logger.info(f"【{self.cookie_id}】全量Cookie采集进度: 字段数={len(merged_cookies)}, 缺少关键字段={missing}")
            if not missing:
                logger.info(f"【{self.cookie_id}】全量Cookie采集后关键字段完整")
                break
    ...
    async def _refresh_cookies_via_browser(self, triggered_by_refresh_token: bool = False):
        ...
        try:
            ...
            missing_keys = self._get_missing_critical_cookie_keys(new_cookies_dict)
            if missing_keys:
                logger.warning(f"【{self.cookie_id}】当前Cookie缺少关键字段: {', '.join(missing_keys)}，开始全量更新")
                full_cookies_dict = await self._collect_full_cookie_snapshot_async(context, page)
                if full_cookies_dict:
                    new_cookies_dict = full_cookies_dict
                    logger.info(f"【{self.cookie_id}】全量更新后Cookie字段数: {len(new_cookies_dict)}")
