        ...
        try:
            ...
            if '<!DOCTYPE html>' in response_text or '<html>' in response_text:
                if '闲鱼' in response_text and ('login' in response_text.lower() or 'mini-login' in response_text):
                    logger.error("❌ 图片上传返回登录页面：可能是Cookie失效，也可能是上传接口风控/请求头限制")
                    logger.error("💡 建议：先继续走Token刷新链路再判定Cookie是否失效，必要时再人工更新Cookie")
                    return None
                else:
                    logger.error(f"收到HTML响应而非JSON，可能是风控或Cookie失效: {response_text[:500]}")
                    return None
