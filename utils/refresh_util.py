...
class DrissionHandler:
    ...
    def set_cookies_from_string(self, cookies_str: str):
        ...
        try:
            ...
            # 解析并支持多行、多段输入（如果包含 Domain=... 则优先使用）
            import re
            cookies_dict = {}
            domain_map = {}  # name -> domain

            # 支持 Set-Cookie 格式，每行可能包含 Domain/Path 等属性
            segments = re.split(r'[\r\n]+|\|\||\|', cookies_str)
            for seg in segments:
                if not seg:
                    continue
                seg = seg.strip()
                # 处理可能的 Set-Cookie: 前缀
                if seg.lower().startswith('set-cookie'):
                    try:
                        _, rest = seg.split(':', 1)
                    except ValueError:
                        rest = seg
                ...
                if '=' in nv:
                    ...
                    for attr in parts[1:]:
                        if attr.lower().startswith('domain='):
                            domain_map[name] = attr.split('=', 1)[1].strip()
                            break

            # 如果没有 domain_map，则为所有cookie尝试设置到常见域名，确保跨域可用
            common_domains = ['.goofish.com', '.m.goofish.com', '.taobao.com', '.m.taobao.com', '.alicdn.com']

            # 设置cookies到浏览器；对每个 cookie，如果解析到 domain 则使用该 domain，否则在 common_domains 上逐个设置
            set_count = 0
            for name, value in cookies_dict.items():
                domains_to_set = [domain_map.get(name)] if name in domain_map else common_domains
                for dom in domains_to_set:
                    ...
                    try:
                        ...
                        logger.debug(f"设置cookie失败 {name} @ {dom}: {e}")

            logger.info(f"已设置 {len(cookies_dict)} 个cookie（尝试设置次数: {set_count}）到浏览器")
    ...
    def get_cookies_string(self) -> str:
        ...
        try:
            ...
            # 按 domain 汇总并包含 domain 信息，便于后续从多个域恢复
            cookie_pairs = []
            for cookie in browser_cookies:
                if isinstance(cookie, dict) and 'name' in cookie and 'value' in cookie:
                    domain = cookie.get('domain', '')
                    cookie_pairs.append(f"{cookie['name']}={cookie['value']}; Domain={domain}")

            cookies_str = '\n'.join(cookie_pairs)
            logger.info(f"获取到 {len(cookie_pairs)} 个cookies（含 domain 信息）")
            return cookies_str
