...
def trans_cookies(cookies_str: str) -> dict:
    ...
    支持以下常见输入格式：
    - 普通的 "k=v; k2=v2"
    - 多行/多段的 Set-Cookie 输出（每行可能包含 Domain/Path/Expires 等属性）
    - JSON 字符串（当配置为结构化数据时）
    - 多段以 `||` 或换行分隔的 cookie 段
    ...
    cookies = {}
    # 先尝试解析为 JSON（兼容从 UI 或导入导出的 JSON 格式）
    try:
        import json as _json
        parsed = _json.loads(cookies_str)
        # 支持 dict 或 list
        if isinstance(parsed, dict):
            # 如果 value 本身是 cookie 字符串，递归解析
            ...
            for item in parsed:
                if isinstance(item, str):
                    try:
                        cookies.update(trans_cookies(item))
                    except Exception:
                        continue
        if cookies:
            return cookies
    ...
    for seg in segments:
        ...
        # 处理可能包含 Set-Cookie: 前缀的行
        low = seg.lower()
        if 'set-cookie' in low:
            # 移除可能的前缀 "Set-Cookie:" 并按逗号或换行拆分多个cookie
            try:
                _, rest = seg.split(':', 1)
            except ValueError:
                rest = seg
            # 有时多个 Set-Cookie 会用逗号隔开，先按逗号再按分号处理
            ...
            for p in parts:
                ...
                # name=value 在第一个分号前
                if '=' in p:
                    name, rest2 = p.split('=', 1)
                    value = rest2.split(';', 1)[0]
                    cookies[name.strip()] = value.strip()
            continue

        # 否则按分号切分为 name=value 项
...
def generate_device_id(user_id: str) -> str:
    """为同一账号生成稳定的设备ID，避免重启后deviceId漂移。"""
    normalized_user_id = str(user_id).strip()
    namespace = uuid.uuid5(uuid.NAMESPACE_DNS, "xianyu-auto-reply")
    stable_uuid = uuid.uuid5(namespace, normalized_user_id)
    return f"{str(stable_uuid).upper()}-{normalized_user_id}"
