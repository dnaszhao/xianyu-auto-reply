...
const statusMap: Record<string, { label: string; class: string }> = {
  processing: { label: '待付款', class: 'badge-warning' },
  ...
  refund_cancelled: { label: '退款已撤销', class: 'badge-info' },
  seller_cancelled: { label: '卖家取消订单', class: 'badge-danger' },
  cancelled: { label: '已关闭', class: 'badge-danger' },
  unknown: { label: '未知', class: 'badge-gray' },
}

const orderSideMap: Record<string, { label: string; class: string }> = {
  sell: { label: '我卖出', class: 'badge-success' },
  buy: { label: '我买入', class: 'badge-warning' },
}
...
export function Orders() {
  ...
  const loadOrders = async (page: number = currentPage) => {
    ...
    try {
      setLoading(true)
      const result = await getOrders(selectedAccount || undefined, selectedStatus || undefined, false, page, pageSize)
      if (result.success) {
    ...
    <div className="space-y-3 sm:space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
        <div>
          ...
          <p className="page-description">查看和管理卖出订单信息</p>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            ...
            <div className="input-group">
              ...
              <Select
                ...
                options={[
                  { value: '', label: '所有状态' },
                  { value: 'processing', label: '待付款' },
                  { value: 'pending_ship', label: '待发货' },
                  { value: 'shipped', label: '已发货' },
                  { value: 'completed', label: '已完成' },
                  { value: 'refunding', label: '退款中' },
                  { value: 'refund_cancelled', label: '退款已撤销' },
                  { value: 'seller_cancelled', label: '卖家取消订单' },
            ...
            <tbody>
              {filteredOrders.length === 0 ? (
                ...
                filteredOrders.map((order) => {
                  ...
                  const orderSide = orderSideMap[order.order_side || 'sell'] || orderSideMap.sell
                  return (
                    <tr key={order.id}>
                      ...
                      <td>
                        <span className={orderSide.class}>{orderSide.label}</span>
                      </td>
