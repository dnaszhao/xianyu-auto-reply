...
async def _execute_password_login(session_id: str, account_id: str, account: str, password: str, show_browser: bool, user_id: int, current_user: Dict[str, Any]):
    ...
    try:
        ...
        def notification_callback(message: str, screenshot_path: str = None, verification_url: str = None, screenshot_path_new: str = None):
            ...
            """
            try:
                # 过滤掉登录容器页这类不可操作链接，避免前端/通知误导用户点击无效地址
                if verification_url:
                    lowered_url = verification_url.lower()
                    blocked_markers = ['mini_login', 'alibaba-login-box', 'newlogin', 'action=captcha', 'purecaptcha']
                    if any(marker in lowered_url for marker in blocked_markers):
                        log_with_user('warning', f"忽略非可用验证链接: {verification_url}", current_user)
...
async def clear_admin_logs(admin_user: Dict[str, Any] = Depends(require_admin)):
    """清空系统日志（管理员专用）"""
    try:
        log_with_user('warning', "清空系统日志", admin_user)
        collector = get_file_log_collector()
        collector.clear_logs()
        log_with_user('info', "系统日志清空成功", admin_user)
        return {"success": True, "message": "日志已清空"}
    except Exception as e:
        log_with_user('error', f"清空系统日志失败: {str(e)}", admin_user)
        return {"success": False, "message": f"清空日志失败: {str(e)}"}
...
async def clear_admin_risk_control_logs(
    ...
    """清空风控日志（管理员专用）"""
    try:
        log_with_user('warning', f"清空风控日志: cookie_id={cookie_id or 'ALL'}", admin_user)
        deleted_count = db_manager.clear_risk_control_logs(cookie_id=cookie_id)
        log_with_user('info', f"风控日志清空成功，删除 {deleted_count} 条记录", admin_user)
        return {
            "success": True,
            "message": f"已清空 {deleted_count} 条风控日志",
            "deleted_count": deleted_count,
        }
    except Exception as e:
    ...
    try:
        ...
        if cookie_id:
            if cookie_id not in allowed_cookie_ids:
                raise HTTPException(status_code=403, detail="无权访问该账号的订单")
            target_cookie_ids = [cookie_id]
        else:
            target_cookie_ids = list(allowed_cookie_ids)

        def _extract_unb(cookie_value: str) -> str:
            try:
                from utils.xianyu_utils import trans_cookies
                return trans_cookies(cookie_value).get('unb', '')
            except Exception:
                return ''
        ...
        for current_cookie_id in target_cookie_ids:
            current_cookie_value = user_cookies.get(current_cookie_id, '')
            current_unb = _extract_unb(current_cookie_value)
            orders = db_manager.get_orders_by_cookie(current_cookie_id, limit=1000)
            # 为每个订单添加cookie_id和方向信息
            for order in orders:
                order['cookie_id'] = current_cookie_id
                buyer_id = str(order.get('buyer_id') or '').strip()
                order_side = 'buy' if current_unb and buyer_id and buyer_id == current_unb else 'sell'
                order['order_side'] = order_side
                if not include_bought and order_side == 'buy':
                    continue
