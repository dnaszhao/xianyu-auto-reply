...
class OrderStatusHandler:
    ...
    # 5. 只有退款完成才设置为已关闭
    VALID_TRANSITIONS = {
        'processing': ['pending_ship', 'shipped', 'completed', 'cancelled', 'seller_cancelled'],
        'pending_ship': ['shipped', 'completed', 'cancelled', 'seller_cancelled', 'refunding'],  # 已付款，可以退款
        'shipped': ['completed', 'cancelled', 'seller_cancelled', 'refunding'],  # 已发货，可以退款
        'completed': ['cancelled', 'seller_cancelled', 'refunding'],  # 已完成，可以退款
        ...
        'seller_cancelled': []  # 卖家取消订单，不能转换到其他状态
    }
    ...
    def __init__(self):
        ...
        self.status_mapping = {
            'processing': '待付款',     # 买家已拍下，等待付款
            'pending_ship': '待发货',   # 已付款，等待发货
            'shipped': '已发货',        # 发货确认后
            'completed': '已完成',      # 交易完成
            'refunding': '退款中',      # 退款中/退货中
            'refund_cancelled': '退款已撤销',  # 退款撤销（临时状态，会回退）
            'cancelled': '已关闭',      # 交易关闭
            'seller_cancelled': '卖家取消订单',  # 卖家主动关闭
        }
    ...
    def _get_status_from_red_reminder(self, red_reminder: str) -> Optional[str]:
        """根据红色提醒内容映射订单状态"""
        reminder = (red_reminder or "").strip()
        if not reminder:
            return None
        ...
        reminder_mapping = {
            ...
            '退款退货中': 'refunding',
            '退款成功': 'cancelled',
            '退款关闭': 'refund_cancelled',
            '退款已关闭': 'refund_cancelled',
        }

        if reminder in reminder_mapping:
            return reminder_mapping[reminder]

        if '退款' in reminder:
            if '撤销' in reminder or '关闭' in reminder:
                return 'refund_cancelled'
            if '成功' in reminder:
                return 'cancelled'
            return 'refunding'

        if '卖家' in reminder and '取消' in reminder:
    ...
    def handle_system_message(self, message: dict, send_message: str, cookie_id: str, msg_time: str) -> bool:
        ...
        try:
            ...
            message_status_mapping = {
                ...
                '你关闭了订单，钱款已原路退返': 'seller_cancelled',
                '[卖家取消订单]': 'seller_cancelled',
                '卖家取消订单': 'seller_cancelled',
            }
            ...
            if current_order and current_order.get('status'):
                ...
                if new_priority < current_priority and new_status not in ['refunding', 'cancelled', 'seller_cancelled']:
    ...
    def handle_red_reminder_message(self, message: dict, red_reminder: str, user_id: str, cookie_id: str, msg_time: str) -> bool:
        ...
        """
        try:
            new_status = self._get_status_from_red_reminder(red_reminder)
            if not new_status:
                return False
            ...
            if success:
                logger.info(f'[{msg_time}] 【{cookie_id}】{red_reminder}，订单 {order_id} 状态已更新为{self.status_mapping.get(new_status, new_status)}')
            else:
            ...
            logger.error(f'[{msg_time}] 【{cookie_id}】处理红色提醒订单状态更新时出错: {str(e)}')
            return False
